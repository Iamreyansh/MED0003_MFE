...
2. Add a `REMOTE_REGISTRY` entry: name `settings`, module `./Mfe`, routes `/settings/profile`, `/settings/storefront`, `/settings/roles`, and `/settings/notifications`.
3. Add a thin page adapter that builds an `MfeDataEnvelope` with `@medmate/settings-contract` and mounts `RemoteLoader`.
4. Wire `/settings/profile`, `/settings/storefront`, `/settings/roles`, and `/settings/notifications` from the registry (do not hardcode strings in nav).
