...
pnpm tf:lock:refresh
cd infra/shared && terraform init -lockfile=readonly && terraform apply
cd ../staging && terraform init -lockfile=readonly && terraform apply
```
...
Provider lockfiles and plugin binaries are platform-specific, so they are not in git. They live in the versioned state bucket `terraform-locks-105927215604`:

| Key                                   | Purpose                                      |
| ------------------------------------- | -------------------------------------------- |
| `MED0003/<stack>/.terraform.lock.hcl` | Provider lockfile (linux + darwin checksums) |
| `MED0003/plugin-cache/`               | Terraform provider plugin cache              |

Pull before `terraform init`. Refresh and upload with `pnpm tf:lock:refresh` when the AWS provider constraint changes.
...
Production keeps the historic state key `MED0003/terraform.tfstate`. Use `./scripts/tf.sh migrate` as the operator checklist when moving ACM/IAM into shared and renaming `module.mfe` to `module.env.module.site`.
