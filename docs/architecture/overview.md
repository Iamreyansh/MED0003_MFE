...
2. **Permanent subdomain per remote** (not per release), with staging at `<name>.staging.mfe.nammamedmate.com`
3. **Immutable release objects** under `/releases/<git-sha>/` plus a checksummed artifact bucket
4. **Atomic promotion** of root `mf-manifest.json` after staging smoke and PDT
...
2. Terraform `for_each` provisions bucket + CloudFront + DNS in staging, then production
3. Release workflow matrix ships only affected packages after staging smoke and PDT
